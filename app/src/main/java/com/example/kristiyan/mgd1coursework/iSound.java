...
public class iSound {
    ...
    iSound(final Context context)
    {
        ...
        try {
            ...
            AssetFileDescriptor afd = ctx.getAssets().openFd("explosion.mp3");
            ...
            //Load sound using the asset file descriptor
            AssetFileDescriptor speedUp = ctx.getAssets().openFd("speedUp.wav");
            //store the id outputted by the sound pool in the sound effects array
            soundIDs[1] = sounds.load(speedUp.getFileDescriptor(), speedUp.getStartOffset(), speedUp.getLength(), 0);
            speedUp.close();
        }
        catch(Exception e) {
            e.printStackTrace();
